# Video Converter · 视频批量压缩工具

> 扫描你的视频库，一键批量压缩——智能评估每个文件的**压缩价值（0-5 星）**，只压缩真正值得压的。
> Batch scan & one-click compress your video library — each file is rated by **compression value (0-5 stars)**, so you only re-encode what's worth it.

基于 PyQt6 + FFmpeg，参照小丸工具箱优化 x264/x265 编码参数，支持硬件加速。A PyQt6 + FFmpeg desktop tool, tuned like Maruko Toolbox.

## 亮点 Highlights

- **压缩价值评分** - 按编码 / 码率 / 体积自动打分，0-100 分映射为 0-5 星，一眼看出哪些文件最值得压缩
- **按价值一键勾选** - 下拉选择 `全部 / >=1星 / >=2星 / >=3星 / >=4星 / >=5星` 自动批量勾选，无需手动挑选
- **多编码器 + 硬件加速** - libx264 / libx265 / VP9 / AV1 + NVIDIA NVENC / Intel QSV；启动时自动检测本机可用编码器
- **真实两遍编码** - 自动按源视频码率计算目标码率（约 60%）执行 ABR 两遍，而非无效的 CRF 两遍
- **体验细节** - 选中统计（体积 / 预计释放空间）、拖拽导入、暂停/继续/停止、压缩后自动关机
...
- **硬件加速** - NVIDIA NVENC、Intel QSV 硬件编码（不可用自动隐藏）
- **两遍编码** - x264/x265 真实 ABR 两遍，目标码率约为源码率 60%
